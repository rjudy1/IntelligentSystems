...
def problem1(hidden_neurons: int):
    ...
    network2 = Network(weight_files=['weights/autoencoderweights_layer0.csv', None],
                       # weight_files=['weights/classifierII0.043_175.csv_layer0.csv', 'weights/classifierII0.043_175.csv_layer1.csv'],
                       neurons_by_layer=[784, hidden_neurons, 10], momentum_alpha=.025, learning_rate=.001,
                       activations_by_layer=[ActivationFunction.RELU, ActivationFunction.LEAKY_RELU])
...
def problem2():
    sofm = SOFM(weight_files=['weights/sofmweights1699978923.547358_layer0.csv'],
        neurons_by_layer=[784, 144], learning_rate=.03, width=12, height=12)
    sofm.train('organizedData/1training', epoch_size=2000, epoch_limit=250, desired_error_rate=-1)

    sofm.write_weights('weights/sofm')

    error, activity_matrix = sofm.test('organizedData/1test')
    print([':'.join([str(i), str([activity_matrix[i][j] for j in range(10)])]) for i in range(10)])
    ...
    feature_map = [[0 for j in range(12)] for i in range(12)]
    for i in range(12):
        for j in range(12):
            feature_map[i][j] = sofm._neurons[0][i*12+j].get_weights()
...
def problem3(sofmweights):
    ...
    network1 = Network(weight_files=[sofmweights, None],
                       neurons_by_layer=[784, 144, 10], momentum_alpha=.025, learning_rate=.001,
                       activations_by_layer=[ActivationFunction.RELU, ActivationFunction.LEAKY_RELU],
...
if __name__ == '__main__':
    ...
    sofm = problem2()
    problem3('weights/sofm_layer0.csv')
